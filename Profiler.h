...
{
    const auto entry = reinterpret_cast<const Profiler::Entry*>(data);
    auto& stage = entry->_stages[idx];
    if (startTimestamp)
    {
        const std::chrono::duration<float, std::milli> fltStart = stage._start - entry->_frameStart;
    ...
    if (endTimestamp)
    {
        *endTimestamp = stage._end.time_since_epoch().count() / 1000000.0f;
        ...
        const std::chrono::duration<float, std::milli> fltEnd = stage._end - entry->_frameStart;
