...
int main(int, char**)
{
    ...
    while (msg.message != WM_QUIT)
    {
        ...
        profiler.Begin(Profiler::Stage::Sort);
        const int size = 5000;
        std::vector<int> vect;
        vect.reserve(size+1);
        for (int i = 0; i < size; i++)
        {
            vect.emplace_back(rand() % 1000);
        }
        std::sort(vect.begin(), vect.end());

        profiler.End(Profiler::Stage::Sort);
        ...
        if (show_profiler_window && !firstFrame)
        {
            ...
            ImGui::SliderInt("slider int", &updates, 1, 30);
            ImGui::BeginChild("Frame", ImVec2(ImGui::GetContentRegionMax().x, 120));
            auto& entry = profiler._entries[profiler.GetCurrentEntryIndex()];
            ImGuiWidgetFlameGraph::PlotFlame("CPU", &ProfilerValueGetter, &entry, Profiler::_StageCount, 0, "Main Thread", FLT_MAX, FLT_MAX, ImVec2(ImGui::GetContentRegionMax().x-64, ImGui::GetContentRegionMax().y));
            ImGui::EndChild();
            ImGuiWidgetFlameGraph::PlotInfo("CPU", &ProfilerValueGetter, &entry, Profiler::_StageCount, 0);
            ImGui::End();
        }
        ...
        const D3DCOLOR clear_col_dx = D3DCOLOR_RGBA(static_cast<int>(clear_color.x*255.0f), static_cast<int>(clear_color.y*255.0f), static_cast<int>(clear_color.z*255.0f), static_cast<int>(clear_color.w*255.0f));
